...
def main():
    ...
    future_dates = pd.date_range(start=grouped2['YearMonthDay'].max() + pd.Timedelta(days=1), periods=n_days)
    ...
    plt.title('Aggregate Daily Forecast: Quantity')
    plt.xlabel('Date')
    plt.ylabel('Quantity')
    plt.legend()
    plt.grid()
    ...
    plt.title('Aggregate Daily Forecast: Revenue')
    plt.xlabel('Date')
    plt.ylabel('Revenue')
    plt.legend()
    plt.grid()
